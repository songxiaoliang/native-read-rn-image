package com.imagetest.splashscreen;
...
//                    }
    ...
    private static int getImageId() {
        int drawableId = getActivity().getResources().getIdentifier("images_icon_splash", "drawable", getActivity().getClass().getPackage().getName());
        if (drawableId == 0) {
            drawableId = getActivity().getResources().getIdentifier("images_icon_splash", "drawable", getActivity().getPackageName());
        }
        return drawableId;
    }

    private void check() {
        Log.e("=-======", CodePush.getServiceUrl());
    }
